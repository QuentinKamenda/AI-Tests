...
def train():
    ...
    model.load_weights('myTTTmodel.h5')
    #makeModel()
    #print("Boardgames is:", np.array(boardgames).shape, "whowon:", np.array(whowon).shape)
    #model.fit(np.array(boardgames), np.array(whowon), epochs=1, validation_split=0.2, shuffle=True, verbose=1, callbacks=callbackS )
    model.save('myTTTmodel.h5')
